...
data_orig=pd.read_excel('/content/indic-swipe/indic-to-indic-datasets/Tamil.xlsx')
lang = 'Tamil'
...
def create_network():
    ...
    dense = TimeDistributed(Dense(60, name="dense"))(encoded_layer) 
...
# Converting predicted sequences into corresponding words using results of LSTM-based decoding
word_pred = []
for i in range(len(pred)):
    next_word = []
    for j in range(len(pred[i])):
...
pred_list_sep = word_pred
corr_list_sep = target_texts_word[TRAIN_SIZE:]
...
comp_model.compile(metrics=['accuracy'], loss='sparse_categorical_crossentropy', optimizer=Adam(lr=0.05))  
...
# ELMo Weighted Average
# Get overall accuracy after the language model
# With 2 dense layers
